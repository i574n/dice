...
def create_sequential_roller(list_1: FSharpList[__A]) -> Callable[[], __A]:
    current_index: int = 0
    def _arrow224(__unit: None=None, list_1: Any=list_1) -> __A:
...
def roll_progressively(log: Callable[[str], None] | None, roll: Callable[[], int], reroll: bool, max: int) -> int:
    def _arrow225(__unit: None=None, log: Any=log, roll: Any=roll, reroll: Any=reroll, max: Any=max) -> int:
        max_1: int = max or 0
        ...
        while True:
            ...
            if size < (power + 1):
                ...
                if pattern_matching_result == 0:
                    ...
                    def _arrow226(_arg: int, rolls: Any=rolls, size: Any=size) -> int:
                        return roll(None)

                    rolls_mut = initialize(power, _arrow226)
                    size_mut = power
                    continue
...
def main(args: Array[str]) -> int:
    def _arrow228(__unit: None=None, args: Any=args) -> Callable[[str], None]:
        clo: Callable[[str], None] = to_console(printf("%s"))
        def _arrow227(arg: str) -> None:
            clo(arg)
    ...
    result: int = roll_progressively(_arrow228(), _arrow230, True, 2147483647 // 10) or 0
