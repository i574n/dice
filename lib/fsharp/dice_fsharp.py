...
def roll_within_bounds(log: Callable[[str], None] | None, max: int, rolls: FSharpList[int]) -> int | None:
    ...
    if match_value is not None:
        def _arrow284(__unit: None=None, log: Any=log, max: Any=max, rolls: Any=rolls) -> bool:
        ...
        while True:
            ...
            if size < (power + 1):
                ...
                if pattern_matching_result == 0:
                    ...
                    def _arrow287(_arg: int, rolls: Any=rolls, size: Any=size) -> int:
                        return roll(None)

                    rolls_mut = initialize(power, _arrow287)
                    size_mut = power
                    continue
...
def main(args: Array[str]) -> int:
    ...
    def _arrow290(__unit: None=None, args: Any=args) -> int:
        return roll_dice()

    result: int = roll_progressively(_arrow289(), _arrow290, True, 2147483647 // 10) or 0
    def _arrow291(__unit: None=None, args: Any=args) -> str:
        return ("main / result: " + str(result)) + ""

    def _arrow292(__unit: None=None, args: Any=args) -> str:
        return ""

    trace(TraceLevel(1), _arrow291, _arrow292)
    return 0
