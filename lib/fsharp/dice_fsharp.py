...
def roll_within_bounds(log: Callable[[str], None] | None, max: int, rolls: FSharpList[int]) -> int | None:
    ...
    if match_value is not None:
        def _arrow228(__unit: None=None, log: Any=log, max: Any=max, rolls: Any=rolls) -> bool:
...
def create_sequential_roller(list_1: FSharpList[__A]) -> Callable[[], __A]:
    current_index: int = 0
    def _arrow229(__unit: None=None, list_1: Any=list_1) -> __A:
...
def roll_progressively(log: Callable[[str], None] | None, roll: Callable[[], int], reroll: bool, max: int) -> int:
    def _arrow230(__unit: None=None, log: Any=log, roll: Any=roll, reroll: Any=reroll, max: Any=max) -> int:
        max_1: int = max or 0
        ...
        while True:
            ...
            if size < (power + 1):
                ...
                if pattern_matching_result == 0:
                    ...
                    def _arrow231(_arg: int, rolls: Any=rolls, size: Any=size) -> int:
                        return roll(None)

                    rolls_mut = initialize(power, _arrow231)
                    size_mut = power
                    continue
...
def main(args: Array[str]) -> int:
    def _arrow233(__unit: None=None, args: Any=args) -> Callable[[str], None]:
        clo: Callable[[str], None] = to_console(printf("%s"))
        def _arrow232(arg: str) -> None:
            clo(arg)

        return _arrow232

    def _arrow234(__unit: None=None, args: Any=args) -> int:
        return roll_dice()

    result: int = roll_progressively(_arrow233(), _arrow234, True, 2147483647 // 10) or 0
    def _arrow235(__unit: None=None, args: Any=args) -> str:
        return ("main / result: " + str(result)) + ""

    def _arrow236(__unit: None=None, args: Any=args) -> str:
        return ""

    trace(TraceLevel(1), _arrow235, _arrow236)
    return 0
